...
def plot_data(db, station):
    ...
    sql = "SELECT time, x, y, z FROM raw_data WHERE device_id=" + station + " ORDER BY time"
    cur.execute(sql)
    ...
    for t in unique_time:
        ...
        plt.plot(time, np.array(x)/10, linewidth=.5, color=[.5, .5, .5]) # x component
        plt.plot(time, np.array(y)/10+1, linewidth=.5, color=[.5, .5, .5]) # y component
        plt.plot(time, np.array(z)/10+2, linewidth=.5, color=[.5, .5, .5]) # z component
