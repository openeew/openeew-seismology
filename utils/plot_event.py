...
def plot_event(ev):
    ...
    title_str = 'Event: ' + ev_name.split('_')[0] +'/'+ ev_name.split('_')[1] +'/'+ ev_name.split('_')[2] + ' M' + str(ev_mag)
    fig.suptitle(title_str, fontsize=16)

    ax1 = plt.subplot2grid((3, 3), (0, 0), colspan=3, rowspan=2, projection=ccrs.PlateCarree())
    ...
    ax1.imshow(prob, extent=[lon_min, lon_max, lat_min, lat_max], cmap='turbo', alpha=.7)
    ax1.coastlines()
    ax1.plot(mine_lon, mine_lat, \
        marker=(5, 1), markersize=12, markerfacecolor=[0.8500, 0.3250, 0.0980], markeredgecolor=[.1, .1, .1])
    ...
    for line in f:
        ...
        if sta in detected_sta:
            ax1.plot(lon, lat, '^', markersize=6, markerfacecolor=[0.8500, 0.3250, 0.0980], markeredgecolor=[.1, .1, .1])
        else:
            ax1.plot(lon, lat, '^', markersize=6, markerfacecolor=[1, 1, 1], markeredgecolor=[.1, .1, .1])


    ax1.plot(ev_lon, ev_lat, marker=(5, 1), markersize=12, markerfacecolor=[1,1,1], markeredgecolor=[.1, .1, .1])
    ...
    ax2.plot(time, y, '-', color=[.8, .8, .8])
    ax2.plot(time, y, 'o', markerfacecolor=[0, 0.4470, 0.7410], markeredgecolor=[.2, .2, .2], markersize=5)
    ax2.set_ylim((0,10))
    ax2.set_xlim((0,60))
    ...
    # Visualize the result   
    ax4.plot(time, dist, '-', color=[.8, .8, .8]) 
