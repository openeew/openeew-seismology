...
def find_and_locate(ev, db, time_now, travel_times, params):
    ...
    ev.get_location(new_detections=new_detections, travel_times=travel_times, db=db, time_now=time_now, params=params)
...
def get_detections(db, time_now, window):
    ...
    sql = "SELECT \
        ...
        FROM detections WHERE time>" + str(time_now-window) + " && time<" + str(time_now)
    cur.execute(sql)
...
class Event():
    ...
    def get_probs_and_associate(self, new_detection, travel_times, db, time_now, params):
        '''
        ...
        if new_device not in detected_devices:
            ...
            for detection in self.detections.values():
                ...
                print('New device: {}, detection device {}'.format(new_device, det_device))

                # get sigma
                sigma = self.get_sigma(new_device, det_device, db, params)

                # calculate probability curve
                tt_prob = np.exp(-(travel_times[det_device] - travel_times[new_device] - det_time + new_time)**2/(2*sigma**2))
            ...
            best_lat, best_lon, _ = self.get_best_location(time_now, travel_times, params=params, save=False, add_prob=new_prob)
            ...
            for detection in self.detections.values():
                ...
                epic_dist_old = self.get_sta_delta(db, det_device_old, eq_lat=best_lat, eq_lon=best_lon)
                epic_dist_new = self.get_sta_delta(db, new_device, eq_lat=best_lat, eq_lon=best_lon)

                # find the closest time from the tt_precalc and place it in the grid
                tt_old = tt_precalc['travel_time'][np.argmin(np.abs(tt_precalc['dist'] - epic_dist_old/111.3))]
                tt_new = tt_precalc['travel_time'][np.argmin(np.abs(tt_precalc['dist'] - epic_dist_new/111.3))]
            ...
            if misfit_mean<assoc_win:
                ...
                self.add_detection(new_detection=new_detection, new_detection_id=new_detection_id)
    ...
    def get_best_location(self, time_now, travel_times, params, save=False, add_prob=0):
        ...
        if save:
            ...
            # append the location
            to_append = {'time': time_now, 'lat': best_lat, 'lon': best_lon, 'depth': best_depth}
    ...
    def get_probs_not_yet_arrived(self, new_detection, travel_times, db, time_now, params):
        '''
        Updates location
        '''
        # get detection variables
        ...
        points = np.concatenate((np.reshape(lon_grid, (lon_grid.size, 1)), np.reshape(lat_grid, (lat_grid.size, 1))), axis=1)
        ...
        best_depth = params['eq_depth'] # depth is fixed for all
        best_orig_time = new_detection['time'] - travel_times['vector']['travel_time'][0]
    ...
    def get_magnitude(self, db, params, time_now):
        ...
        conf2 = self.magnitude['grid'][np.argmin(abs(cum_prob-.02))]
        conf16 = self.magnitude['grid'][np.argmin(abs(cum_prob-.16))]
        conf84 = self.magnitude['grid'][np.argmin(abs(cum_prob-.84))]
        conf98 = self.magnitude['grid'][np.argmin(abs(cum_prob-.98))]

        # form the tuple and append to the list of magnitudes
        mag_tuple = {'time': time_now, 'mag': magnitude, \
            'mag_conf2': conf2, 'mag_conf16': conf16, 'mag_conf84': conf84, 'mag_conf98': conf98}
    ...
    def prior_mag(self, params):
        ...
        mag_step = .01; mag_min = 0; mag_max = 10
        ...
        if prior_type=='gutenberg':
            ...
            mag_prob[peak_index:] = max_value - np.arange(0, max_value, max_value/num_of_steps)

            # the linear increase to the Mc
            num_of_steps = int(2 * (1/mag_step))
            mag_prob[peak_index-num_of_steps:peak_index] = np.arange(0, max_value, max_value/num_of_steps)
    ...
    def get_pd_detection_id(self, db, det_id):
        ...
        sql = "SELECT \
            ...
            FROM detections WHERE detection_id=" + str(det_id)
        cur.execute(sql)
    ...
    def add_detection(self, new_detection, new_detection_id):
        ...
        unique_device = list(set([dev['device_id'] for dev in self.detections.values()]))
    ...
    def get_active_devices(self, db, time_now):
        ...
        # get the detections less than 10 s old
        sql = "SELECT device_id FROM raw_data WHERE time>" + str(time_now-2) + " && time<" + str(time_now)
        cur.execute(sql)
    ...
    def get_sta_delta(self, db, sta, **kwargs):
        ...
        sql = "SELECT \
            latitude, longitude \
            FROM devices WHERE device_id=" + sta
        cur.execute(sql)
    ...
    def num_of_dets(self):
        ...
        number_of_detections = len([n['assoc'] for n in self.detections.values() if n['assoc']==True])
