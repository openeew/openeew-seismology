...
def calculate_tt(tt_precalc, grid, sta_lat, sta_lon, eq_depth):
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            tt[i, j] = tt_precalc['travel_time'][np.argmin(np.abs(tt_precalc['dist'] - distance_in_degree))]
...
def calculate_trave_times(db, params):
    ...
    if calculate_open=='calculate':
        ...
        grid = make_grid(lat_min=lat_min, lat_max=lat_max, lon_min=lon_min, lon_max=lon_max, step=step)
        ...
        for device in devices:
            ...
            travel_time[sta_name] = calculate_tt(tt_precalc, grid, sta_lat, sta_lon, eq_depth)
