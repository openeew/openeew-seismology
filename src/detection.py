...
def detection2db(db, device_id, det_time, params):
    ...
    sql = (
        "SELECT device_id, time FROM detections WHERE time>"
        + str(det_time - det_off_win)
        + " && time<"
        + str(det_time + det_off_win)
        + " && device_id="
        + device_id
...
def get_pd(trace, time, det_time, params):
    ...
    sos = signal.butter(4, (0.2, 3), "bandpass", fs=samp_freq, output="sos")
...
def station_magnitude(data_array, db, time_now, params):
    ...
    # get the detections less than 10 s old
    sql = (
        "SELECT detection_id, device_id, time FROM detections WHERE time>"
        + str(time_now - 10)
        + " && time<"
        + str(time_now)
    )
    ...
    for det in detections:
        ...
        try:
            ...
            sta_ind = [
                index for index, elem in enumerate(data_array["sta"]) if elem == det[1]
            ]
            chan_ind = [
                index
                for index, elem in enumerate(data_array["chan"])
                if elem == vert_chan
            ]
...
def detect(model, data_buffer, db, time_now, params):
    ...
    if params["det_type"] == "ml":
        ...
        # do ML detection
        detect_stalta(data_array=data_array, db=db, time_now=time_now, params=params)

    # do station magnitude
    station_magnitude(data_array=data_array, db=db, time_now=time_now, params=params)
